...
(async () => {
  ...
  async function loadProducts(){const {data,error}=await db.from('products').select('id,slug,platform,category,pricing,name_en,name_ko,tagline_en,tagline_ko,description_en,description_ko,website_url,tags,staff_pick,featured,published_at,visit_count,review_count,profiles!products_owner_id_fkey(display_name)').eq('status','published').order('published_at',{ascending:false});if(error){console.error('VibeGuys product load failed',error);return}products=data||[]}
