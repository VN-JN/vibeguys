  ...
  function render(){
    ...
    app.innerHTML=`<main class="reference-home home-v3">
      ...
      <section class="home-directory">
        ...
        <div class="home-directory-column"><header><div><h2><span>♨</span> POPULAR</h2><p>${t('실제 반응이 좋은 서비스','Products with real momentum')}</p></div><button data-view="trending">${t('모두 보기','View all')} ›</button></header><div class="home-rank-list">${rankRows([products[6],products[0],products[2],products[1],products[3]])}</div></div>
        <div class="home-directory-column"><header><div><h2><span>▣</span> WEB &amp; APP</h2><p>${t('웹, 앱, 혹은 둘 다 제공하는 서비스','Products available on web, app, or both')}</p></div><button data-view="explore">${t('모두 보기','View all')} ›</button></header><div class="home-type-list">${[products[4],products[2],products[5],products[0],products[6]].map(product=>`<article data-a="detail" data-id="${product.id}"><img src="${product.image}" alt=""><strong>${escape(product.name)}</strong><span>${platformLabel(product.platform)}</span></article>`).join('')}</div></div>
